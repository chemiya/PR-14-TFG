...
const buscarRecetasUsuario=async(req,res)=>{
    const { id } = req.params
    let sql = `(select r.id,r.dificultad,u.fotoRuta as fotoCreador, r.titulo, u.username as usernameUsuario, r.tiempo,r.fotoRuta from receta r,usuario u where r.idCreador=u.id and  u.id='${id}');`//hago select de todos
    conexion.query(sql, (err, rows, fields) => {
        if (err) throw err;
        else {
...
const buscarRecetaPorId = async (req, res) => {
    ...
    let sql = `select r.id, r.titulo,u.fotoRuta as fotoCreador, r.resumen,r.tiempo,r.dificultad, u.username as usernameUsuario, r.idCreador, r.fotoRuta from receta r, usuario u where r.idCreador=u.id and r.id= '${id}' `//hago select de todos
    conexion.query(sql, (err, rows, fields) => {
        if (err) throw err;
        else {
            res.json(rows)//devuelvo el resultado en json
...
const actualizarReceta = async (req, res) => {
    ...
    try {
        ...
        const { id } = req.params
        const { titulo, resumen, dificultad, tiempo } = req.body//cojo los campos y el id que me llega y hago actualizaciony devuelvo texto
...
module.exports={
    actualizarReceta,eliminarReceta,crearReceta,buscarRecetaPorId,buscarRecetas,buscarRecetasUsuario
}